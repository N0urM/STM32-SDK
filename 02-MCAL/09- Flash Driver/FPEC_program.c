...
void FPEC_voidLock() {
	 ...
	 */
	SET_BIT(FPEC -> CR, FLASH_CR_LOCK);
}

void FPEC_voidLockOptionBytes()
{
    /* Software Reset */
    CLR_BIT(FPEC -> CR , FLASH_CR_OPTWRE);
}
...
{
    if(GET_BIT (FPEC ->  SR , FLASH_SR_WRPRTERR))
        return WRITE_PROTECTION_ERROR;
    if (GET_BIT (FPEC -> SR , FLASH_SR_PGERR))
